...
void KEY_Task(void *parameter)
{
    ...
    evt.type = INPUT_KEY_SERVICE_ACTION_CLICK;
    uint8_t a = INPUT_KEY_USER_ID_REC;
    evt.data = &a;
    while (1) {
        if (state == 0) {
            ...
            state = 0;
            input_key_service_cb(NULL, &evt, (void *)http_stream_writer);
            xEventGroupSetBits(EXIT_FLAG, (0x01 << 0));
            vTaskDelay(20 * 1000);
        }
    }
}
