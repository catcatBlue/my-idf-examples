...
/*
 This code displays some fancy graphics on the LV_HOR_RES_MAXxLV_VER_RES_MAX LCD on an ESP-WROVER_KIT board.
...
// but less overhead for setting up / finishing transfers. Make sure LV_VER_RES_MAX is dividable by this.
...
void lcd_init(spi_device_handle_t spi)
{
    ...
    // ili9481_set_orientation(3);
    const char *orientation_str[] = {
        "PORTRAIT", "PORTRAIT_INVERTED", "LANDSCAPE", "LANDSCAPE_INVERTED"
    };
    ESP_LOGI(TAG, "Display orientation: %s", orientation_str[3]);
    uint8_t data2[] = {0x48, 0x4B, 0x28, 0x2B};
    lcd_cmd(spi, ILI9481_CMD_MEMORY_ACCESS_CONTROL);
    lcd_data(spi, &data2[3], 1);
...
static void send_lines(spi_device_handle_t spi, int ypos, uint16_t *linedata)
{
    ...
    trans[1].tx_data[2] = (LV_HOR_RES_MAX) >> 8;                      // End Col High
    trans[1].tx_data[3] = (LV_HOR_RES_MAX) & 0xff;                    // End Col Low
    ...
    trans[5].length     = LV_HOR_RES_MAX * 2 * 8 * PARALLEL_LINES;    // Data length, in bits
...
static void display_pretty_colors(spi_device_handle_t spi)
{
    ...
    for (int i = 0; i < 2; i++) {
        lines[i] = heap_caps_malloc(LV_HOR_RES_MAX/*  * 2  */* PARALLEL_LINES * sizeof(uint16_t), MALLOC_CAP_DMA);
        assert(lines[i] != NULL);
    }
    int frame = 0;
    ...
    while (1) {
        frame++;
        for (int y = 0; y < LV_VER_RES_MAX/* -10 */; y += PARALLEL_LINES) {
            // Calculate a line.
...
void example_task1(void *pvParameters)
{
    ...
    spi_bus_config_t    buscfg = {
           ...
           .quadwp_io_num   = -1,
           .quadhd_io_num   = -1,
           .max_transfer_sz = PARALLEL_LINES * LV_HOR_RES_MAX * 2 + 8};
